      ...
      (compilation, { normalModuleFactory }) => {
        normalModuleFactory.hooks.parser
          ...
          .tap("MiniProgramGlobalWebpackPlugin", (parser) => {
            ...
            parser.hooks.expression.for("miniProgramRequire").tap(
              "MiniProgramGlobalWebpackPlugin",
              /** @param {MemberExpressionNode} expression */ (expression) => {
                const dep = new ConstDependency(`require`, expression.range);
                dep.loc = expression.loc;
                parser.state.module.addPresentationalDependency(dep);
                return true;
              },
