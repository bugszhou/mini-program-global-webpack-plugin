      ...
      (compilation, opts) => {
        ...
        normalModuleFactory.hooks.parser.for("javascript/auto").tap(
          ...
          (parser) => {
            ...
            parser.hooks.callMemberChain.for("miniProgramRequire").tap(
              "MiniProgramGlobalWebpackPlugin",
              /** @param {import("webpack/lib/javascript/JavascriptParser").CallExpression} expression */ (
                expression,
              ) => {
                const params = expression.arguments;
                const dep = new ConstDependency(
                  `require.async(__mini_program_path__ + ${params[0].raw}).then(function(){return ${params[0].raw};})`,
                  expression.range,
                );
                dep.loc = expression.loc;
                parser.state.module.addPresentationalDependency(dep);
            ...
            parser.hooks.evaluateCallExpressionMember.for("async").tap(
              ...
              /** @param {import("webpack/lib/javascript/JavascriptParser").CallExpression} expression */ (
                ...
                if (params.identifier === "miniProgramRequire") {
                  const dep = new ConstDependency(
                    `require.async(__mini_program_path__ + ${expression.arguments[0].raw}).then(function(){return ${params[0].raw};});`,
                    expression.range,
                  );
                  dep.loc = expression.loc;
                  parser.state.module.addPresentationalDependency(dep);
